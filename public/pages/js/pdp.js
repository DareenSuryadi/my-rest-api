function getDataByWindowUrlKey(){
    ...
    let url = baseUrl+'/api/art/'+windowUrlKey;
    ...
    .then(function (response) {
      ...
      $('#product-art_name').html(response.data.art_name);
      ...
      $('#product-artist').html(response.data.artist);
      $('#product-techniques').html(response.data.techniques);
      ...
      // START -- note, unless you have these data in a database structure, here we are hardcoding them for display purposes
        ...
        // tag
        let collectionOfTag = ['Abstract','Nature','History','Mythology','Social','Political','Spiritual','Fantasy','Futuristic','Life','Love','Journey'];
